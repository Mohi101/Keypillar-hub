...
export default async function OverviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const section = sections.find(s => s.sectionName === "Project Overview");
    ...
    <div>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="font-semibold text-foreground">Project Overview</p>
            <p className="text-sm text-muted-foreground">Section content</p>
          </div>
          <span className={`badge ${badge(section?.status || "MISSING")}`}>{section?.status || "Missing"}</span>
        </div>
        <p className="text-muted-foreground text-sm">
          {section?.purpose || "No content yet. Use this section to document the project overview."}
        </p>
      </div>
    </div>
  );
