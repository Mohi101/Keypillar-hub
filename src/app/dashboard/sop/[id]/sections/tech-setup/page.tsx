...
export default async function TechSetupPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const project = await prisma.project.findUnique({ where: { id } });
  const sections = await prisma.projectSection.findMany({ where: { projectId: id } });
  const section = sections.find(s => s.sectionName === "Technical Setup Guide");
  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-title">Technical Setup Guide</h1>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="font-semibold text-foreground">Technical Setup Guide</p>
